...
def get_lstm():
    ...
    model.add(keras.layers.LSTM(10, activation='relu', input_shape=input_shape))
...
for train_index, test_index in kf.split(folds):
    ...
    trainloss, trainacc = model.evaluate(x_train, y_train, verbose=verbosity)
    valloss, valacc = model.evaluate(x_val, y_val, verbose=verbosity)
...
print("Average Train 5 Folds Accuracy: {0}".format(np.mean(trainaccuracies)))
print("Average Val 5 Folds Accuracy: {0}".format(np.mean(valaccuracies)))
